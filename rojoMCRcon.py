...
class MCRcon(object):
    ...
    def _listen(self, out_type):
        ...
        while True:
            ...
            # Record the response
            in_data += in_data_partial.decode("utf8", 'ignore')

            # If there's nothing more to receive, return the response
            if len(select.select([self.socket], [], [], 0)[0]) == 0:
                return in_data

    def _sendNoResponse(self, out_type, out_data):
        if self.socket is None:
            raise MCRconException("Must connect before sending data")

        # Send a request packet
        out_payload = (
            struct.pack("<ii", 0, out_type) + out_data.encode("utf8", 'ignore') + b"\x00\x00"
        )
